function nameSpace(param){
	...
	funcSpace[param]={
			...
			openDetailWindow:function(){
				openPopWindow_cattle(this.json_id[saveKey],this.json_name[saveKey]);
			},
			//定义自己的格式化函数
...
//						+ 'id:'+json_id[key]
						+'<br/>' + this.key_arr[2] + ':'+ new Date(x).Format("yyyy/MM/dd") + '<br/>'
			...
			chartScatter:function(data,containerId,chartJson) {
				...
				//step5循环data  push数据
			 	$.each(data,function(i,value){			 		
			 		...
			 		x = getNumVal(value[that.key_arr[2]]);
			 		
			 		
			 		y = getNumVal(value[that.key_arr[3]]);
					...
					if(index == -1){
						...
						//step9 初始化新增种类对象
						 var simpleSeries={
				            ...
				            name:promise,//胎次
				            color: const_color[index%(const_color.length-1)],//颜色
				            data:[]//数据集合          
						 };	
						var pos=[x,y];
