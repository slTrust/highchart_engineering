function nameSpace(param){
	...
	funcSpace[param]={
			...
			drillMap:{},
			chartStackColumn:function(data,containerId,chartJson) {	
				...
				series = [];
				series.push({'name':titles[1], 'type':'column','yAxis':1,'data':[]});
				for(var i=2; i < titles.length; i++){
					series.push({'name':titles[i], 'type':'column','yAxis':1,'data':[]});
